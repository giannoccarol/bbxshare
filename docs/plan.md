...
2. **Ricezione** — COMPLETATA end-to-end su Q10 (2026-08-28):
   ...
   - ricezione streaming di file e payload testo in
     ...
     progress UI e rimozione dei file parziali in caso di errore.
...
3. **Invio** — IMPLEMENTATA lato client con la versione `0.4.1.1`:
   ...
   - discovery mDNS dei receiver vicini con query a burst, cache dei record
     frammentati e annunci periodici; elenco selezionabile con nomi leggibili;
   - handshake Quick Share client UKEY2, consenso remoto e streaming cifrato
     AES-256-CBC/HMAC-SHA256 verso il device scelto;
